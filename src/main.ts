...
export async function run(args: {
  ...
  const port = Number(Deno.env.get("PORT")) || args.port || 8000;
  delete args.port;
      ...
      if (req.method == "GET") {
        if (url.protocol == "http:" || url.protocol == "https:") {
          ...
          if (req.headers.get("accept")?.includes("application/nostr+json")) {
            ...
            const relay_info =
              await args.relayInformationStore.resolveRelayInformation();
...
export const software = "https://github.com/xdamman/relayed";

const graphql_login_handler =
  (args: { relayInformationStore: RelayInformationStore }) =>
  async (req: Request) => {
...
async function verifyToken(
  ...
  const storeInformation =
    await relayInformationStore.resolveRelayInformation();
  if (storeInformation instanceof Error) {
